...
use MaxDev\Enums\AppointmentStatus;
...
class Updated implements ShouldBroadcast, ShouldQueue
{
    ...
    {
        if ($this->appointment->isDirty('status')) {
    ...
    {
        $this->delete();
        info('Appointment updated customer support event failed: ' . exception_message($exception));
        exit;
    }
}
